...
class Operator(ev.OperatorBase):
    ...
    def eval(self, inputs: List[ev.OperandInput]):
        ...
        contours, _ = cv2.findContours(closed_canny_data, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

        # Create an output mask to draw the detected DSLs
        mask = np.zeros_like(closed_canny_data)
        cv2.drawContours(mask, contours, -1, (255), thickness=cv2.FILLED)

        # Dilation to broaden the mask areas
        dilated_mask = cv2.dilate(mask, np.ones((20, 20), np.uint8), iterations=1)
